...
public class PhoneBook {
	...
	public boolean updateName(String name, String updateName) {
		...
		int index = list.indexOf(new PhoneNumber(name, ""));//익명객체 만들어준다
		//없으면 수정이 안된 => 메서드 종료
		if(index == -1) {
			return false;
		}
		//찾은 전화번호의 이름을 수정
		//전화번호 부에서 이름과 일치하는 전화번호를 가져옴
		PhoneNumber pn = list.get(index);
		//가져온 전화번호의 이름을 수정
		pn.setName(updateName);
		//list.get(index).setName(updateName);//=>위의 두줄 아래와 같이 한줄로 수정 가능
		...
		//침조변수는 =을 이용하면 주소값을 복사하기 때문에 한쪽의 값이 바뀌면 다른쪽도 바뀜(공유함)
		//예를 들면 pn1에 저장된 주소가 0x10000
	...
	public boolean updateNumber(String name, String number) {
		 ...
		 * 있으면 name과 number를 이용하여 전화번호 객체를 생성 => 생성하는 대신 checkNumber를 이용
		 * PhoneNumber class의 checkNumber를 public static로 바꿈*/
		//number가 전화번호 패턴이면 전화번호르 수정
	...
	public boolean update(String name, String updateName, String number) {
		...
		//기존에 있는 전화번호를 삭제
		list.remove(index);
		//updateName과 number를 이용히여 전화번호 객체를 생성한 후 추가
		pn.setName(updateName);
		list.add(pn);
		return true;		
	}
