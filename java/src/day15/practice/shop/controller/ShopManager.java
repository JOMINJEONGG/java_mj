...
public class ShopManager implements RunInterface{
	...
	private void sell() {
		...
		//=> 이렇게 안하면 창고에 있는 제품를 뜻하여 제고량을 알수 있는 것임
	...
	private void printProduct() {				
		//검색할 제품을 입력
		System.out.print("제품명 : ");
		sc.nextLine();//이전 입력에서 엔터를 쳤기 때문에 제거
		String name = sc.nextLine();
		...
		if(index >= 0) {
			list[index].print();
			return;//이게 없으면 아래 else로 문장처리해야함 취향에 맞게하기~
		}
		//없으면 없는 제품이라고 출력
	...
	private void registerCustomer() {
		...
		//고객을 등록(고객 리스트에)
		//이미 등록된 전화번호이면 등록을 X
		for(int i = 0; i < customerCount ; i++) {//고객 조회
			//customerList[i] : 고객 리스트에서 i번지에 있는 고객정보
