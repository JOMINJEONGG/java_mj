...
public class VocabularyNote {
	...
	public VocabularyNote(Word wordList[]) {
		//기존 단어장의 크기와 10을 비교해서 큰수로 단어장 크키로 선택
	 ...
	 * 찾는 단어가 없으면 보통 -1을 return으로 표현함
	 * 음수는 원래 나올 수 없는 번지이기 때문에 음수로~
	 * 매개변수 : 단어 => String title
	 * 리턴타입 : 위치 => 번지 => int 
	 * 메스드명 : indexOf => 번지 찾을때 index 많이 사용
	 * */
	...
	/**단어가 주어지면 단어장에 해당 단어를 출력하는 메서드 - 검색기능
	 * 매개변수 : 단어 => String title
	 * 리턴타입 : 없음 => void
	 * 메스드명 : search
	 * */
	 ...
	 * 매개변수 : 단어와 수정할 단어가 주어지면 => String title, String updateTitle
	 * 리턴타입 : 없음 = > void
	 * 메스드명 : updateTitle
	 * */
